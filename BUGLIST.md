...
| B3 | 2026-06-22 | Hoog | Onderzoek (diagnose-build 0.0.2.1) | **scrcpy-server start/luistert niet — hardware-test.** Eerste echte test op Z Fold 6 → cartablet: zowel `FULL_USB` als `USB_BOOTSTRAP_WIFI` falen identiek. Goed: USB-connect + RSA-auth + **jar-push slagen** (komt tot `PUSHING_SERVER`). Fout: `kon niet verbinden met de scrcpy-server-socket` (`connectWithRetry` 20× faalt op `localabstract:scrcpy`) → de server-jar start niet of luistert niet. Wij lazen de server-output niet → blind. **Fix-stap:** `ScrcpyServer.start` vangt nu stdout/stderr van de `shell:`-start en toont die in de foutmelding (0.0.2.1). Verdachten: corrupte handgerolde sync-push (ClassNotFound), onbekende v3.1-server-optie, SELinux/app_process, of socket-naam (`scid`→`scrcpy_xxxx`). Volgende test toont de echte oorzaak. |
