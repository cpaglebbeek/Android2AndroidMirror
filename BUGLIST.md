...
| B3 | 2026-06-22 | Hoog | **Opgelost (0.0.2.2)** | **scrcpy-server crashte op `video_bit_rate=2M`.** Hardware-test Z Fold 6 → cartablet: USB-connect + RSA-auth + jar-push slaagden, maar `kon niet verbinden met de scrcpy-server-socket`. Diagnose-build 0.0.2.1 (server-output opvangen) onthulde de oorzaak: `java.lang.NumberFormatException: For input string: "2M"` in `Options.parse` → de scrcpy-**server** verwacht `video_bit_rate` als integer (bits/sec); de "2M"-suffix wordt door de desktop-*client* omgerekend, niet de server. Wij stuurden `2M` rechtstreeks door → server stopte direct → `localabstract:scrcpy` werd nooit aangemaakt. **Fix:** `Profile.videoBitRate` `"2M"` → `"2000000"`. Diagnostische output-capture blijft (nuttig voor de volgende laag). |
